...
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(uint8_t w, uint8_t h, TwoWire *twi,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(uint8_t w, uint8_t h, int8_t mosi_pin,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(uint8_t w, uint8_t h, SPIClass *spi,
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(int8_t mosi_pin, int8_t sclk_pin,
                                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(int8_t dc_pin, int8_t rst_pin, int8_t cs_pin)
...
/*!
    ...
    @note   Call the object's begin() function before use -- buffer
            allocation is performed there!
*/
FiberPunk_SSD1306::FiberPunk_SSD1306(int8_t rst_pin)
...
*/
FiberPunk_SSD1306::~FiberPunk_SSD1306(void) {
  if (buffer) {
    free(buffer);
    buffer = NULL;
  }
...
inline void FiberPunk_SSD1306::SPIwrite(uint8_t d) {
  if (spi) {
    (void)spi->transfer(d);
  } else {
...
void FiberPunk_SSD1306::ssd1306_command1(uint8_t c) {
  if (wire) { // I2C
...
void FiberPunk_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if (wire) { // I2C
    wire->beginTransmission(i2caddr);
    WIRE_WRITE((uint8_t)0x00); // Co = 0, D/C = 0
    uint16_t bytesOut = 1;
...
/*!
    ...
    @return None (void).
*/
void FiberPunk_SSD1306::ssd1306_command(uint8_t c) {
  TRANSACTION_START
  ssd1306_command1(c);
  TRANSACTION_END
}
...
*/
bool FiberPunk_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
                             bool periphBegin) {
...
*/
void FiberPunk_SSD1306::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void FiberPunk_SSD1306::clearDisplay(void) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void FiberPunk_SSD1306::drawFastHLine(int16_t x, int16_t y, int16_t w,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
void FiberPunk_SSD1306::drawFastHLineInternal(int16_t x, int16_t y, int16_t w,
                                             uint16_t color) {
...
/*!
    ...
    @note   Changes buffer contents only, no immediate effect on display.
            ...
            commands as needed by one's own application.
*/
void FiberPunk_SSD1306::drawFastVLine(int16_t x, int16_t y, int16_t h,
                                     uint16_t color) {
  bool bSwap = false;
  switch (rotation) {
  case 1:
...
void FiberPunk_SSD1306::drawFastVLineInternal(int16_t x, int16_t __y,
...
*/
bool FiberPunk_SSD1306::getPixel(int16_t x, int16_t y) {
  if ((x >= 0) && (x < width()) && (y >= 0) && (y < height())) {
    // Pixel is in-bounds. Rotate coordinates if needed.
    switch (getRotation()) {
    case 1:
...
*/
uint8_t *FiberPunk_SSD1306::getBuffer(void) { return buffer; }
...
*/
void FiberPunk_SSD1306::display(void) {
  TRANSACTION_START
...
void FiberPunk_SSD1306::startscrollright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void FiberPunk_SSD1306::startscrollleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void FiberPunk_SSD1306::startscrolldiagright(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
void FiberPunk_SSD1306::startscrolldiagleft(uint8_t start, uint8_t stop) {
  TRANSACTION_START
...
/*!
    ...
    @return None (void).
*/
void FiberPunk_SSD1306::stopscroll(void) {
  TRANSACTION_START
...
*/
void FiberPunk_SSD1306::invertDisplay(bool i) {
  TRANSACTION_START
...
*/
void FiberPunk_SSD1306::dim(bool dim) {
